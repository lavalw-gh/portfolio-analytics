...
def build_holdings_overview_full(
    ...
    """
    Build the same Holdings Overview table as the UI, but always with full values
    (no masking) so it can be used in reports.
    Returns (hold_table, total_val, port_ytd, bench_ytd).
    """
    ...
    return hold_table, total_val, float(port_ytd), float(bench_ytd)
...
def generate_pdf_report(
    ...
    """
    Generate landscape A4 PDF report with charts and tables.
    Returns PDF bytes.
    """
    ...
    elements.append(
        Paragraph(
            f"Portfolio YTD: {port_ytd:.2%} | "
            f"{benchmark} YTD: {bench_ytd:.2%}",
            styles["Normal"],
        )
    )

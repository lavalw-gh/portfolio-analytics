...
st.set_page_config(page_title="Portfolio Analyzer 2.13", layout="wide")
...
def normalize_and_rebase_to_pounds(
    ...
    for ticker in tickers:
        try:
            ...
            try:
                ...
                long_name = info_dict.get("longName") or info_dict.get("shortName")
            except Exception:
                long_name = None
    ...
    with st.expander(
        "📋 Price Normalization, Rebasing & Name Lookup Log", expanded=True
    ):
...
def compute_risk_metrics(twr_daily: pd.Series, rf_daily):
    ...
    downside_dev_annual = (
        downside_dev_daily * np.sqrt(252) if np.isfinite(downside_dev_daily) else np.nan
    )
...
def main():
    st.title("📊 Portfolio Analyzer 2.13 - uses Yahoo Data")
    st.warning(
    ...
    date_preset = st.sidebar.selectbox(
        "Date range",
        ["Custom", "Last 3 months", "Last 6 months", "YTD", "Last 12 months", "Last 24 months"],
        index=4,
    ...
    # Resolve effective requested start_date before applying earliest-tx clamp
                ...
                with st.expander("Open scenario sandbox"):
                    ...
                    if not scen_table.empty:
                        ...
                        if not scen_ret.empty:
                            ...
                            base_cum_rebased = base_cum - base_cum.iloc[0]

                            scen_cum_full = scen_cum.reindex(base_cum.index).ffill()
                            if not scen_cum_full.empty:
                                scen_cum_rebased = scen_cum_full - scen_cum_full.iloc[0]
                            else:
                                scen_cum_rebased = scen_cum_full

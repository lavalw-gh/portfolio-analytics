# Portfolio Analyzer (Streamlit): loads transaction CSVs and normalises all costs/prices to GBP.
...
# ============================================================================
# YAHOO PRICE SPIKE CLEANING (v2.21)
# ============================================================================
...
def clean_daily_spikes_flat(
    ...
    Args:
        close: DataFrame of close prices (index=dates, columns=symbols)
        threshold: Maximum allowed 1-day % move (default 0.25 = 25%)

    Returns:
        (clean_close, corrections) where corrections is list of dicts with:
...
# v2.20: Inject constant £1.00 price series for CASH$ (cash holdings)
        ...
        for s in all_symbols:
            ...
            if series.isna().all():
                ...
                if first_valid is not None and first_valid > first_index:
                    ...
                    # v2.20: Backfill missing leading period using the first available price
...
def add_holdings_totals_row(hold_table: pd.DataFrame) -> pd.DataFrame:
    ...
    total_shares = float(pd.to_numeric(
        hold_table["Shares"], errors="coerce").fillna(0.0).sum())
    total_cost = float(pd.to_numeric(
        hold_table["Cost (£)"], errors="coerce").fillna(0.0).sum())
    total_value = float(pd.to_numeric(
        hold_table["Value (£)"], errors="coerce").fillna(0.0).sum())

    totals = {c: 0.0 for c in hold_table.columns}
    if "Ticker" in totals:
        totals["Ticker"] = "TOTAL"
    if "Name" in totals:
        totals["Name"] = "Total"

    totals["Shares"] = total_shares
    totals["Cost (£)"] = total_cost
    totals["Value (£)"] = total_value

    return pd.concat([hold_table, pd.DataFrame([totals])], ignore_index=True)
...
def main():
            ...
            try:
                ...
                for idx, pname in enumerate(selected_portfolios):
                    if pname in portfolio_data:
                        ...
                        hold_table = add_holdings_totals_row(hold_table)
    ...
    try:
        ...
        for pname in selected_portfolios:
            ...
            # v2.20: Totals row for Shares/Cost/Value (always included in exports)
            hold_table = add_holdings_totals_row(hold_table)
    ...
    # =========================================================================
    # EXPORT ALL CHARTS (SIDEBAR)  (v2.20)
    # =========================================================================
    ...
    with tabs[0]:
        ...
        if selected_for_cum:
            ...
            if cum_data:
                ...
                # v2.21: Data quality notes
                st.markdown("---")
                st.subheader("📋 Data Quality Notes")

                # Get metadata from price_df
                spike_corrections = getattr(price_df, "_spike_corrections", []) or []
                missing_ranges_meta = getattr(price_df, "_missing_ranges", []) or []

                # Group corrections by symbol
                corrections_by_symbol = {}
                for corr in spike_corrections:
                    sym = corr.get("symbol", "?")
                    if sym not in corrections_by_symbol:
                        corrections_by_symbol[sym] = []
                    corrections_by_symbol[sym].append(corr)

                # Group missing ranges by symbol
                missing_by_symbol = {}
                for m in missing_ranges_meta:
                    sym = m.get("symbol", "?")
                    if sym not in missing_by_symbol:
                        missing_by_symbol[sym] = []
                    missing_by_symbol[sym].append(m)

                # Display notes if there are any issues
                has_issues = bool(corrections_by_symbol or missing_by_symbol)

                if has_issues:
                    # Missing prices section
                    if missing_by_symbol:
                        st.markdown("**⚠️ Missing Price Data (Backfilled)**")
                        for sym in sorted(missing_by_symbol.keys()):
                            ranges = missing_by_symbol[sym]
                            for m in ranges:
                                if m.get("type") == "leading_nan":
                                    st.markdown(
                                        f"- **{sym}**: Missing from {m.get('start', '?')} to {m.get('end', '?')}; "
                                        f"backfilled using price from {m.get('used_price_from', '?')}"
                                    )
                                elif m.get("type") == "no_data":
                                    st.markdown(
                                        f"- **{sym}**: No usable Yahoo data between {m.get('start', '?')} and {m.get('end', '?')}"
                                    )
                        st.markdown("")

                    # Spike corrections section
                    if corrections_by_symbol:
                        st.markdown("**🔧 Price Spikes Normalized**")
                        for sym in sorted(corrections_by_symbol.keys()):
                            corr_list = corrections_by_symbol[sym]
                            dates_str = ", ".join([
                                c.get("date").strftime("%d/%m/%Y") if hasattr(c.get("date"), "strftime") 
                                else str(c.get("date", "?"))
                                for c in corr_list
                            ])
                            st.markdown(
                                f"- **{sym}**: {len(corr_list)} spike(s) flattened on: {dates_str}"
                            )
                        st.markdown("")

                    st.caption(
                        "**Note**: Missing prices were backfilled using the first available price to avoid "
                        "artificial performance jumps. Price spikes (>25% daily move) were normalized by "
                        "carrying forward the previous day's price."
                    )
                else:
                    st.success("✅ No data quality issues detected. All prices are complete and spike-free.")
    ...
    with tabs[4]:
        ...
        if selected_holding:
            ...
            hold_table_display = add_holdings_totals_row(
                hold_table) if show_values else hold_table

            if show_values:
                # Normal numeric formatting
                styled_hold = hold_table_display.style.format(
                    {
